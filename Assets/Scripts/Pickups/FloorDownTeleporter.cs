...
/// 玩家走到该格子时拾取，下楼传送器数量 +1，随后由玩家按 Z 键使用。
/// </summary>
...
public class FloorDownTeleporter : MonoBehaviour
{
    ...
    public bool TryPickup(PlayerData playerData)
    {
        if (playerData == null)
        {
            Debug.LogError($"[FloorDownTeleporter] playerData 为 null，无法拾取");
            return false;
        }

        playerData.AddDownTeleporter(1);
        ...
        Debug.Log($"[FloorDownTeleporter] 拾取下楼传送器（第 {floorNumber} 层）");
        Destroy(gameObject);
        return true;
    }
